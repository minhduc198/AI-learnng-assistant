...
export default function ChatInterface() {
  ...
  const renderMessage = (msg, index) => {
    ...
    return (
      <div
        key={index}
        className={`flex items-center gap-3 my-4 ${isUser ? "justify-end" : ""}`}
      >
        {!isUser && (
  ...
  return (
    <div className="flex flex-col h-[70vh] bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl py-6 shadow-xl shadow-slate-200/50 overflow-hidden">
      {/* Messages Area */}
      ...
      <div className="p-5 border-t border-slate-200/60 bg-white/80">
        <form onSubmit={handleSendMessage} className="flex items-center gap-3">
          ...
          <button
            ...
            className="shrink-0 flex items-center justify-center w-12 h-12 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25 disabled:opacity-50 disabled:cursor-not-allowed active:scale-95  "
          >
